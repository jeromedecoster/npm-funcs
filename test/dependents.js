...
test('dependents', { timeout:20000 }, function (t) {
  ...
  .then(function(data) {
    ...
    var keys = Object.keys(data.packages)
    t.deepEqual(keys.length > 1, true)
    t.deepEqual(typeof data.packages[keys[0]].version,    'string')
    t.deepEqual(typeof data.packages[keys[0]].dependency, 'string')
    t.deepEqual(typeof data.packages[keys[0]].outdated,   'boolean')
    t.end()
  })
  .catch(function(err) {
...
test('dependents unknown package', { timeout:10000 }, function (t) {
